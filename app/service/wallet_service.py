...
from app.schemas.schemas import WalletCreate, WalletResponse, WalletBalanceResponse, WalletDelete, WalletUpdate
from app.service.exhange_service import get_exchange_rate


async def get_wallet(session: AsyncSession, user_id:int, wallet_name: str) -> WalletBalanceResponse:
    ...
    if wallet is None:
        raise WalletNotFound()
    return WalletBalanceResponse(name=wallet_name, balance=wallet.balance, currency=wallet.currency)
...
async def get_total_balance(user_id: int, session: AsyncSession) -> WalletBalanceResponse:
    ...
    return WalletBalanceResponse(name="Total Balance", balance=total_balance, currency=CurrencyEnum.RUB)
